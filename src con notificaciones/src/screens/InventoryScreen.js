...
import { Search, Package, Trash, Filter, X, AlertCircle, ZoomIn, Share2 } from 'lucide-react-native';
...
export default function InventoryScreen() {
  ...
  // ── Zoom con PanResponder (100% React Native, sin librerías) ─
  //const scaleAnim = useRef(new Animated.Value(1)).current;
  const currentScale = useRef(1);
  const initialDistance = useRef(null);
  const lastTap = useRef(0);
  const liveScale = useRef(1); // ✅ sigue el valor en tiempo real
  ...
  const shareImage = async () => {
    ...
    try {
      // Si la imagen ya es local (file://) la compartimos directo
      if (selectedImage.startsWith('file://')) {
        await Sharing.shareAsync(selectedImage);
        return;
      }

      // Si es una URL remota, la descargamos primero
        ...
        {filteredMeds.length === 0 ? (
          ...
          <>
            <Text style={styles.resultsText}>
              {filteredMeds.length}{' '}
              {filteredMeds.length === 1 ? 'medicamento' : 'medicamentos'} encontrados
            </Text>
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: 'white', borderRadius: 16, marginBottom: 16, padding: 16, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4 },
  ...
  deleteButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 8, borderTopWidth: 1, borderTopColor: '#E5E7EB', marginTop: 8, gap: 8 },
  deleteButtonText: { color: '#DC2626', fontWeight: '600' },
  reactivarButton: { backgroundColor: '#10B981', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 8, borderRadius: 8, marginTop: 8 },
