...
import { Package, AlertCircle, ChevronDown, ChevronUp, Trash, Activity, Key } from 'lucide-react-native';
import { getDaysUntilExpiry } from '../utils/dateUtils';
//import * as Notifications from 'expo-notifications';
import { checkAndNotifyExpiringMedicines } from '../services/NotificationService.js';
import { Bell } from 'lucide-react-native';
import { forceCheckNotifications } from '../services/NotificationService.js';

export default function HomeScreen({ navigation, onOpenApiKeyModal }) {
    ...
    <ScrollView style={styles.container}>
        ...
        <View style={styles.headerRow}>
          ...
          <TouchableOpacity 
            ...
            onPress={async () => {
              ...
              Alert.alert('Notificaciones', 'Verificación completada. Revisa la consola.');
            }}
          >
            <Bell color="white" size={18} />
            <Text style={styles.testNotificationText}>Probar notificaciones</Text>
          </TouchableOpacity>
          <TouchableOpacity 
      ...
      {medicamentosPorVencer.length > 0 && (
        <View style={styles.section}>
          ...
          {showPorVencer && medicamentosPorVencer.map(med => (
            <MedicamentoCard 
              key={med.id} 
              med={med} 
              status="porVencer"
      ...
      {medicamentosVencidos.length > 0 && (
        <View style={styles.section}>
          ...
          {showVencidos && medicamentosVencidos.map(med => (
            <MedicamentoCard 
              key={med.id} 
              med={med} 
              status="vencido"
      ...
      <View style={styles.medInfo}>
        ...
        <View style={styles.medFooter}>
          ...
          <Text style={[styles.medExpiry, { color: colors.text }]}>
            {status === 'vencido' ? 'Venció: ' : 'Vence: '}{new Date(med.vencimiento).toLocaleDateString()}
          </Text>
        </View>
      </View>
    </View>
